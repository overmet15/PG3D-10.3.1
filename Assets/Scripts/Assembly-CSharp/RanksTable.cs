...
public class RanksTable : MonoBehaviour
{
	...
	private FonTableRanksController team1Table;

	private FonTableRanksController team2Table;
	...
	private void UpdateRanksFromTabs()
	{
		...
		if (ConnectSceneNGUIController.regim == ConnectSceneNGUIController.RegimGame.TeamFight || ConnectSceneNGUIController.regim == ConnectSceneNGUIController.RegimGame.FlagCapture || ConnectSceneNGUIController.regim == ConnectSceneNGUIController.RegimGame.CapturePoints)
		{
			for (int i = 0; i < 6; i++)
			{
				if (i < tabsBlue.Count || (!flag2 && totalBlue > sumBlue))
				{
					if (i < tabsBlue.Count && i < 5)
					{
						...
						playersBut[i + ((num == 2) ? 6 : 0)].UpdateState(true, isMine, (num > 0) ? ((num == 1) ? 1 : 2) : 0, component.NamePlayer, text2, text, component.myRanks, component.myClanTexture, pixelbookID);
					}
					else if (ConnectSceneNGUIController.regim != ConnectSceneNGUIController.RegimGame.CapturePoints)
					{
						playersBut[i + ((num == 2) ? 6 : 0)].UpdateState(true, false, (num > 0) ? ((num == 1) ? 1 : 2) : 0, othersStr, string.Empty, (totalBlue - sumBlue).ToString(), -1, null, string.Empty);
						flag2 = true;
					}
					else
					{
						playersBut[i + ((num == 2) ? 6 : 0)].UpdateState(false, false, 0, string.Empty, string.Empty, string.Empty, 1, null, string.Empty);
					}
				}
				else
				{
				...
				if (i < tabsRed.Count || (!flag3 && totalRed > sumRed))
				{
					if (i < tabsRed.Count && i < 5)
					{
						...
						playersBut[i + ((num != 2) ? 6 : 0)].UpdateState(true, isMine2, (num > 0) ? ((num == 2) ? 1 : 2) : 0, component2.NamePlayer, text4, text3, component2.myRanks, component2.myClanTexture, pixelbookID2);
					}
					else if (ConnectSceneNGUIController.regim != ConnectSceneNGUIController.RegimGame.CapturePoints)
					{
						playersBut[i + ((num != 2) ? 6 : 0)].UpdateState(true, false, (num > 0) ? ((num == 2) ? 1 : 2) : 0, othersStr, string.Empty, (totalRed - sumRed).ToString(), -1, null, string.Empty);
						flag3 = true;
					}
					else
					{
			...
			NetworkStartTableNGUIController.sharedController.totalBlue.text = ((num == 2) ? totalRed.ToString() : totalBlue.ToString());
			NetworkStartTableNGUIController.sharedController.totalRed.text = ((num == 2) ? totalBlue.ToString() : totalRed.ToString());
			return;
		}
		for (int j = 0; j < 12; j++)
		{
			...
			for (int i = 0; i < 6; i++)
			{
				if (i < networkStartTable.oldSpisokNameBlue.Length || (!flag2 && totalBlue > sumBlue))
				{
					if (i < networkStartTable.oldSpisokNameBlue.Length && i < 5)
					{
						...
						playersBut[i + ((num == 2) ? 6 : 0)].UpdateState(true, isMine, (num > 0) ? ((num == 1) ? 1 : 2) : 0, networkStartTable.oldSpisokNameBlue[i], text2, text, networkStartTable.oldSpisokRanksBlue[i], networkStartTable.oldSpisokMyClanLogoBlue[i], pixelbookID);
					}
					else if (ConnectSceneNGUIController.regim != ConnectSceneNGUIController.RegimGame.CapturePoints)
					{
						int num3 = ((totalBlue > sumBlue) ? (totalBlue - sumBlue) : 0);
						sumBlue += num3;
						playersBut[i + ((num == 2) ? 6 : 0)].UpdateState(true, false, (num > 0) ? ((num == 1) ? 1 : 2) : 0, othersStr, string.Empty, num3.ToString(), -1, null, string.Empty);
						flag2 = true;
					}
					else
					{
				...
				if (i < networkStartTable.oldSpisokNameRed.Length || (!flag3 && totalRed > sumRed))
				{
					if (i < networkStartTable.oldSpisokNameRed.Length && i < 5)
					{
						...
						playersBut[i + ((num != 2) ? 6 : 0)].UpdateState(true, isMine2, (num > 0) ? ((num == 2) ? 1 : 2) : 0, networkStartTable.oldSpisokNameRed[i], text4, text3, networkStartTable.oldSpisokRanksRed[i], networkStartTable.oldSpisokMyClanLogoRed[i], pixelbookID2);
					}
					else if (ConnectSceneNGUIController.regim != ConnectSceneNGUIController.RegimGame.CapturePoints)
					{
						int num5 = ((totalRed > sumRed) ? (totalRed - sumRed) : 0);
						sumRed += num5;
						playersBut[i + ((num != 2) ? 6 : 0)].UpdateState(true, false, (num > 0) ? ((num == 2) ? 1 : 2) : 0, othersStr, string.Empty, num5.ToString(), -1, null, string.Empty);
						flag3 = true;
					}
					else
					{
