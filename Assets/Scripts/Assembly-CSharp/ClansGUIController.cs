...
public sealed class ClansGUIController : MonoBehaviour, IFriendsGUIController
{
	...
	private IEnumerator __UpdateGUI()
	{
		...
		foreach (FriendPreview fp2 in array)
		{
			bool found = false;
			foreach (Dictionary<string, string> member2 in FriendsController.sharedController.clanMembers)
			{
				string _id;
				if (!member2.TryGetValue("id", out _id) || !_id.Equals(fp2.id))
				{
					continue;
				}
				found = true;
				fp2.nm.text = member2["nick"];
				break;
			}
			if (!found)
			{
			...
			{
				existingPreviews.Add(fp2.id);
			}
		}
		foreach (FriendPreview fp in toRemove)
		{
		...
		{
			if (member.ContainsKey("id") && !existingPreviews.Contains(member["id"]) && !member["id"].Equals(FriendsController.sharedController.id))
			{
				...
				f.GetComponent<FriendPreview>().id = member["id"];
				...
				{
					f.GetComponent<FriendPreview>().nm.text = member["nick"];
				}
			}
		}
		yield return null;
	...
	{
		if (_clanIncomingInvitesController.Value.Catch((ClanIncomingInvitesController c) => c.inboxPanel.activeInHierarchy))
		{
	...
	private void Update()
	{
		...
		{
			receivingPlashka.SetActive(!NoClanPanel.activeInHierarchy && !CreateClanPanel.activeInHierarchy && !clanPanel.activeInHierarchy && !statisiticPanel.activeInHierarchy && !addInClanPanel.activeInHierarchy && (CurrentState != State.Inbox || (CurrentState == State.Inbox && ClanIncomingInvitesController.CurrentRequest.Filter((Task<List<object>> t) => t.IsCompleted) == null)));
