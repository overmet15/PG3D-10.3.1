...
public class KillRateCheck
{
	...
	public static KillRateCheck instance
	{
		get
		{
			...
			if (!_instance.configLoaded)
			{
				_instance.LoadParameters();
				Debug.LogWarning("Config not loaded: try loading");
			}
			return _instance;
		}
	}
	...
	private void LoadParameters()
	{
		...
		{
			Debug.LogError("KillRateCheck: key not found (BuffsParam)");
		...
		{
			Debug.LogError("Error Deserialize JSON: BuffsParam");
			WriteDefaultParameters();
			return;
		}
		try
		{
			...
			if (list == null)
			{
				Debug.LogError("Error Deserialize JSON: tierParams");
				return;
			}
		...
		{
			Debug.LogError("Error Deserialize JSON: BuffsParam: " + ex);
			WriteDefaultParameters();
			return;
		}
		configLoaded = true;
	...
	{
		instance.WriteStatusAndResetCounter(CheckStatus.BuyedGun);
	}

	public static void OnGunTakeOff()
	{
		instance.WriteStatusAndResetCounter(CheckStatus.CoolDown);
	}

	private void WriteStatusAndResetCounter(CheckStatus stat)
	{
		roundCount = 0;
		status = stat;
		SaveValues();
