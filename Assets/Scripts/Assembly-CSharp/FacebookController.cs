...
public class FacebookController : MonoBehaviour
{
	...
	public bool IsNeedShowGunFroLoginWindow()
	{
		...
		return socialGunEventActive && @int < 1 && Application.loadedLevelName.Equals(Defs.MainMenuScene) && Storager.getInt(Defs.IsFacebookLoginRewardaGained, true) == 0;
	}
	...
	internal static void CheckAndGiveFacebookReward(string context)
	{
		if (FacebookSupported)
		{
			...
			if (FacebookSupported && Storager.getInt(Defs.IsFacebookLoginRewardaGained, true) == 0 && Storager.getInt(Defs.FacebookRewardGainStarted, false) == 1 && FB.IsLoggedIn)
			{
				...
				dictionary.Add("Login Facebook", context);
				Dictionary<string, string> dictionary2 = dictionary;
				FlurryPluginWrapper.LogEventAndDublicateToEditor("Virality", dictionary2, new Color32(59, 89, 152, byte.MaxValue));
				FlurryPluginWrapper.LogEventToAppsFlyer("Virality", dictionary2);
