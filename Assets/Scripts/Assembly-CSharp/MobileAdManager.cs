...
internal sealed class MobileAdManager
{
	...
	public static bool UserPredicate(Type adType, bool verbose, bool showToPaying = false, bool showToNew = false)
	{
		...
		if (adType == Type.Video)
		{
			...
			if (verbose)
			{
				Debug.Log(string.Format("AdIsApplicable ({0}): {1}    Paying: {2},  Need to show: {3},  Session count satisfied: {4},  Lobby level: {5}", adType, flag9, flag2, (!flag2) ? flag6 : flag5, flag8, lobbyLevel));
			}
		}
		else
		{
	...
	{
		byte b = GuidBytes[0];
		return (b % 2 != 0) ? SampleGroup.Video : SampleGroup.Image;
	}
