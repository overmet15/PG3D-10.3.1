...
internal sealed class ProfileController : MonoBehaviour
{
			...
			if (updateWeapon && WeaponManager.sharedManager != null)
			{
				...
				if (array.Length > 0)
				{
					...
					else
					{
						...
						profileView.SetWeaponAndSkin(weapon.weaponPrefab.tag, false);
					}
				}
				else
				{
	...
	public static string GetPlayerNameOrDefault()
	{
		...
		if (Defs.AndroidEdition == Defs.RuntimeAndroidEdition.Amazon && GameCircleSocial.Instance != null && GameCircleSocial.Instance.localUser != null && GameCircleSocial.Instance.localUser.authenticated && !string.IsNullOrEmpty(GameCircleSocial.Instance.localUser.userName))
		{
