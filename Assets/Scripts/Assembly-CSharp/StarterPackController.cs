...
internal sealed class StarterPackController : MonoBehaviour
{
	...
	private bool TryTakePurchases(string productId, int packOrder, bool isRestore = false)
	{
		...
		if (!isRestore)
		{
			FlurryEvents.LogSales((!StoreKitEventListener.inAppsReadableNames.ContainsKey(productId)) ? productId : StoreKitEventListener.inAppsReadableNames[productId], "Starter Pack");
		}
		return true;
	}
