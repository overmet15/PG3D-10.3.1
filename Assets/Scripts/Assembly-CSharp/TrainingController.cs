...
public sealed class TrainingController : MonoBehaviour
{
	...
	public GameObject killZombiesOverlay6s;
	...
	private IEnumerator Start()
	{
		...
		_overlays = new GameObject[11]
		{
			swipeToRotateOverlay, dragToMoveOverlay, pickupGunOverlay, wellDoneOverlay, getCoinOverlay, enterShopOverlay, shootReloadOverlay, selectGrenadeOverlay, throwGrenadeOverlay, killZombiesOverlay,
			killZombiesOverlay6s
		};
		isPause = false;
		...
		if (InGameGUI.sharedInGameGUI.jumpButton != null && pickupGunOverlay != null)
		{
			...
			InGameGUI.sharedInGameGUI.jumpButton.GetComponentsInChildren(true, sprites);
			TrainingBlinking tb3 = pickupGunOverlay.AddComponent<TrainingBlinking>();
			tb3.SetSprites(sprites);
		}
		if (InGameGUI.sharedInGameGUI.fireButton != null)
		{
			List<UISprite> sprites2 = new List<UISprite>();
			InGameGUI.sharedInGameGUI.fireButton.GetComponentsInChildren(true, sprites2);
			if (killZombiesOverlay6s != null)
			{
				TrainingBlinking tb2 = killZombiesOverlay6s.AddComponent<TrainingBlinking>();
				tb2.SetSprites(sprites2);
			}
			if (killZombiesOverlay != null)
			{
				TrainingBlinking tb = killZombiesOverlay.AddComponent<TrainingBlinking>();
				tb.SetSprites(sprites2);
			}
		}
	}
	...
	public void StartNextStepTraning()
	{
		...
		{
			FlurryEvents.LogTutorial("2_Controls_Overview");
		...
		{
			FlurryEvents.LogTutorial("3_Controls_Move");
			isCanceled = true;
			maxStepAnim = 19;
			speedAnim = 0.5f;
			stepAnim = 0;
		...
		{
			FlurryEvents.LogTutorial("4_Controls_Jump");
		...
		{
			FlurryEvents.LogTutorial("5_Shooting");
		...
		{
			FlurryEvents.LogTutorial("6_Kill_Enemy");
			if (_enemies.Count > 0)
			{
		...
		{
			FlurryEvents.LogTutorial("7_Portal");
			if (_directionArrow.Value != null)
			{
	...
	private void RefreshOverlays()
	{
		...
		{
			objA = ((!Input.touchPressureSupported) ? killZombiesOverlay : killZombiesOverlay6s);
		}
