...
internal sealed class BankView : MonoBehaviour, IDisposable
{
	...
	private void LoadIconForItem(BankViewItem item, int i, bool isGems, bool load)
	{
		if (load)
		{
			string text = ((!isGems) ? ("Textures/Bank/Coins_Shop_" + (i + 1)) : ("Textures/Bank/Coins_Shop_Gem_" + (i + 1)));
	...
	{
		List<BankViewItem> list = (goldItemGrid.GetComponentsInChildren<BankViewItem>(true) ?? new BankViewItem[0]).ToList();
		for (int i = 0; i < list.Count; i++)
		{
			LoadIconForItem(list[i], list[i].purchaseInfo.Index, false, load);
		}
		List<BankViewItem> list2 = (gemsItemGrid.GetComponentsInChildren<BankViewItem>(true) ?? new BankViewItem[0]).ToList();
	...
	private void UpdateItem(BankViewItem item, int i, bool isGems)
	{
		...
		if (purchaseEventArgs.Index < array.Length)
		{
			...
			if (Defs.ABTestDeviceNumber == 2)
			{
				purchaseEventArgs.Count *= 3;
			}
		...
		string text2 = ((!isGems) ? ("Textures/Bank/Coins_Shop_" + (i + 1)) : ("Textures/Bank/Coins_Shop_Gem_" + (i + 1)));
		if (Device.IsLoweMemoryDevice)
		{
			PreloadTexture component = item.icon.GetComponent<PreloadTexture>();
			if (component != null)
			{
				component.pathTexture = text2;
			}
		}
		else
		{
			item.icon.mainTexture = Resources.Load<Texture>(text2);
		}
