			...
			if (bossShowm)
			{
				...
				if (!Defs.IsSurvival)
				{
					...
					if (!LevelBox.weaponsFromBosses.ContainsKey(Application.loadedLevelName))
					{
						...
						if (gameObject2 != null)
						{
							PlayerArrowToPortalController[] array = UnityEngine.Object.FindObjectsOfType<PlayerArrowToPortalController>();
							PlayerArrowToPortalController[] array2 = array;
							foreach (PlayerArrowToPortalController playerArrowToPortalController in array2)
							{
								playerArrowToPortalController.SetPointOfInterest(gameObject2.transform);
							}
						}
						return;
					}
					...
					if (gameObject3 != null)
					{
						PlayerArrowToPortalController[] array3 = UnityEngine.Object.FindObjectsOfType<PlayerArrowToPortalController>();
						PlayerArrowToPortalController[] array4 = array3;
						foreach (PlayerArrowToPortalController playerArrowToPortalController2 in array4)
						{
							playerArrowToPortalController2.SetPointOfInterest(gameObject3.transform);
						}
					}
				}
				else
	...
	private void Awake()
	{
		...
		if (Defs.IsSurvival)
		{
			...
			foreach (List<string> enemiesInWave in _enemiesInWaves)
			{
				...
				{
					if (!_allEnemiesSurvival.Contains(item))
					{
