...
internal sealed class RewardedLikeButton : MonoBehaviour
{
	...
	public void OnClick()
	{
		...
		try
		{
			...
			if (Storager.getInt("RewardForLikeGained", true) <= 0)
			{
				...
				FlurryPluginWrapper.LogEventAndDublicateToConsole("Virality", new Dictionary<string, string> { { "Like Facebook Page", "Likes" } });
	...
	internal void Refresh()
	{
		...
		else if (MainMenuController.sharedController.Catch((MainMenuController m) => m.socialGunPanel.gameObject.activeInHierarchy))
		{
