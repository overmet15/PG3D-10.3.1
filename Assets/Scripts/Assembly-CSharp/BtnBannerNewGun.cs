...
public class BtnBannerNewGun : ButtonBannerBase
{
	...
	public override void OnUpdateParameter()
	{
		...
		if (!string.IsNullOrEmpty(tagForClick))
		{
			...
			lbSale.text = string.Format("{0}\n{1}%", LocalizationStore.Key_0419, PromoActionsManager.sharedManager.discounts[tagForClick][0]);
			lbPrice.text = PromoActionsManager.sharedManager.discounts[tagForClick][1].ToString();
