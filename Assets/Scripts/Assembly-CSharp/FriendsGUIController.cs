...
public sealed class FriendsGUIController : MonoBehaviour, IFriendsGUIController
{
	...
	private IEnumerator __UpdateGUI()
	{
		...
		{
			if (!existingClanInvs.Contains(ClanInv["id"]))
			{
				...
				if (ClanInv.ContainsKey("id"))
				{
					f.GetComponent<Invitation>().id = ClanInv["id"];
					f.GetComponent<Invitation>().recordId = ClanInv["id"];
				}
				if (ClanInv.ContainsKey("name"))
				{
					f.GetComponent<Invitation>().nm.text = ClanInv["name"];
				}
				string clanLogo;
				if (ClanInv.TryGetValue("logo", out clanLogo) && !string.IsNullOrEmpty(clanLogo))
				{
