...
public class FriendsWindowController : MonoBehaviour
{
	...
	private IEnumerator UpdateCurrentFriendsList()
	{
		...
		if (!wrapsInit)
		{
			...
			{
				GameObject friendPreviewItem3 = NGUITools.AddChild(inviteFriendsWrap.gameObject, friendPreviewPrefab);
			...
			for (int f = 0; f < 6; f++)
			{
				GameObject friendPreviewItem = NGUITools.AddChild(findFriendsWrap.gameObject, friendPreviewPrefab);
	...
	public IEnumerator ShowResultFindPlayer(string param)
	{
		...
		if (friendsController != null)
		{
			FlurryPluginWrapper.LogEventAndDublicateToConsole("Social", new Dictionary<string, string> { { "Search Friends", "Search" } });
