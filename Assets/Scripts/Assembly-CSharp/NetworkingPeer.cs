...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
	...
	protected internal string mAppVersionPun
	{
		get
		{
			return string.Format("{0}_{1}", PhotonNetwork.gameVersion, "1.61");
		}
	}
	...
	private void ReadoutProperties(ExitGames.Client.Photon.Hashtable gameProperties, ExitGames.Client.Photon.Hashtable pActorProperties, int targetActorNr)
	{
		...
		{
			CurrentGame.CacheProperties(gameProperties);
	...
	public bool SetMasterClient(int nextMasterId)
	{
		...
		return OpSetPropertiesOfRoom(gameProperties, false, expectedProperties);
	}
	...
	public void OnStatusChanged(StatusCode statusCode)
	{
		...
		case StatusCode.EncryptionEstablished:
			...
			if (!didAuthenticate && (!IsUsingNameServer || CloudRegion != CloudRegionCode.none))
			{
				...
				didAuthenticate = OpAuthenticate(mAppId, mAppVersionPun, authValues2, CloudRegion.ToString(), requestLobbyStatistics);
				if (didAuthenticate)
				{
					State = global::PeerState.Authenticating;
				}
		...
		case StatusCode.EncryptionFailedToEstablish:
		{
			...
			AuthenticationValues authValues = authenticationValues;
			OpAuthenticate(mAppId, mAppVersionPun, authValues, CloudRegion.ToString(), requestLobbyStatistics);
			break;
		}
	...
	public void OnEvent(EventData photonEvent)
	{
		...
		case 206:
		{
			...
			for (short num6 = num5; num6 < hashtable3.Count; num6++)
			{
				OnSerializeRead(hashtable3[num6] as ExitGames.Client.Photon.Hashtable, photonPlayer, networkTime, correctPrefix);
			}
			return;
		}
		case 202:
		...
		case 207:
		{
			...
			int num7 = (int)hashtable2[(byte)0];
			if (num7 >= 0)
			{
		...
		{
			ExitGames.Client.Photon.Hashtable hashtable2 = (ExitGames.Client.Photon.Hashtable)photonEvent[245];
			int num4 = (int)hashtable2[(byte)0];
