...
public sealed class FriendsController : MonoBehaviour
{
	...
	private IEnumerator GetPixelbookSettingsLoop()
	{
	...
	private IEnumerator GetNewsLoop()
	{
	...
	private IEnumerator GetBuffSettings()
	{
		WWW response = Tools.CreateWwwIfNotConnected(URLs.BuffSettings1031);
		if (response == null)
		{
			yield break;
		}
	...
	{
		WWW response = Tools.CreateWwwIfNotConnected(URLs.LobbyNews);
		if (response == null)
		{
			yield break;
		}
		...
		List<Dictionary<string, object>> oldNewsAll = (Json.Deserialize(oldNews) as List<object>).OfType<Dictionary<string, object>>().ToList();
		List<Dictionary<string, object>> newsAll = (Json.Deserialize(responseText) as List<object>).OfType<Dictionary<string, object>>().ToList();
		if (newsAll.Count == 0)
		{
			isAnyNews = false;
		}
	...
	{
		WWW response = Tools.CreateWwwIfNotConnected(URLs.PixelbookSettings);
		if (response == null)
		{
			yield break;
		}
	...
	public IEnumerable<float> InitController()
	{
		...
		UpdatePixelbookSettingsFromPrefs();
		StartCoroutine(GetPixelbookSettingsLoop());
		StartCoroutine(GetNewsLoop());
		...
		StartCoroutine(GetBuffSettings());
		...
		{
			Debug.Log("Account id:    " + id + "    Calling CheckOurIdExists()...");
	...
	private IEnumerator OnApplicationPause(bool pause)
	{
		...
		if (Application.loadedLevelName.Equals("Friends") && FriendsGUIController.ShowProfile && FriendProfileController.currentFriendId != null && readyToOperate)
		{
	...
	public void SendInvitation(string personId, Dictionary<string, string> socialEventParameters)
	{
		...
		try
		{
			...
			if (shouldSendKillRate)
			{
				...
				foreach (string weapon in keysIntersection)
				{
					...
					if (weaponInfo.tag == null)
					{
						Debug.LogError("Exception adding kill_rate to update_player: weaponInfo.tag == null  " + weapon);
						continue;
					}
					if (weapon == WeaponManager.SocialGunWN || WeaponManager.GotchaGuns.Contains(weaponInfo.tag))
					{
	...
	private IEnumerator GetClanDataLoop()
	{
		while (true)
		{
			if (idle || !Application.loadedLevelName.Equals("Clans") || string.IsNullOrEmpty(ClanID))
			{
				yield return null;
				continue;
			}
	...
	public IEnumerator FriendRequest(string personId, Dictionary<string, string> socialEventParameters, Action<bool, bool> callbackAnswer = null)
	{
		if (socialEventParameters == null)
		{
			throw new ArgumentNullException("socialEventParameters");
		...
		if (response.Equals("ok"))
		{
			...
			FlurryPluginWrapper.LogEventAndDublicateToConsole("Social", socialEventParameters);
		}
		...
		if (dataList.ContainsKey("local_users"))
		{
			...
			if (userList2 != null && userList2.Count > 0)
			{
				foreach (Dictionary<string, object> dictListItem3 in userList2.OfType<Dictionary<string, object>>())
				{
					...
					{
						Dictionary<string, object> _cache5 = profileInfo[userId3]["player"] as Dictionary<string, object>;
						_cache5["nick"] = clone3["nick"];
						_cache5["rank"] = clone3["rank"];
						_cache5["clan_logo"] = clone3["clan_logo"];
						_cache5["clan_name"] = clone3["clan_name"];
						_cache5["skin"] = clone3["skin"];
						profileInfo[userId3]["player"] = _cache5;
					}
					else
					{
						Dictionary<string, object> _cache5 = new Dictionary<string, object>
						{
							{
								"nick",
								clone3["nick"]
						...
						Dictionary<string, object> _infoPlayer3 = new Dictionary<string, object> { { "player", _cache5 } };
		...
		if (dataList.ContainsKey("facebook_users"))
		{
			...
			if (facebookUsers != null && facebookUsers.Count > 0)
			{
				foreach (Dictionary<string, object> dictListItem2 in facebookUsers.OfType<Dictionary<string, object>>())
				{
					...
					{
						Dictionary<string, object> _cache3 = profileInfo[userId2]["player"] as Dictionary<string, object>;
						_cache3["nick"] = clone2["nick"];
						_cache3["rank"] = clone2["rank"];
						_cache3["clan_logo"] = clone2["clan_logo"];
						_cache3["clan_name"] = clone2["clan_name"];
						_cache3["skin"] = clone2["skin"];
						profileInfo[userId2]["player"] = _cache3;
					}
					else
					{
						...
						Dictionary<string, object> _infoPlayer2 = new Dictionary<string, object> { { "player", _cache3 } };
		...
		if (dataList.ContainsKey("users"))
		{
			...
			if (userList != null && userList.Count > 0)
			{
				foreach (Dictionary<string, object> dictListItem in userList.OfType<Dictionary<string, object>>())
				{
					...
					{
						Dictionary<string, object> _cache = profileInfo[userId]["player"] as Dictionary<string, object>;
						...
						profileInfo[userId]["player"] = _cache;
					}
					else
					{
						...
						Dictionary<string, object> _infoPlayer = new Dictionary<string, object> { { "player", _cache } };
	...
	public static void SendFriendshipRequest(string playerId, Dictionary<string, string> socialEventParameters, Action<bool, bool> callbackResult)
	{
		if (!(sharedController == null))
		{
			if (socialEventParameters == null)
