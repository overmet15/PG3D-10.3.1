...
public class SorryWeaponAndArmorBanner : BannerWindow
{
	...
	public override void Show()
	{
		...
		{
			_compensationGoldCount.Value = Storager.getInt("RemovedArmorHat_CoinsCountToCompensate", false);
			_compensationGemsCount.Value = 0;
		}
		else if (sorryGearRemoved)
		{
			_compensationGoldCount.Value = 0;
			_compensationGemsCount.Value = Storager.getInt(Defs.GearRemoved_GemsCountToCompensate, false);
		}
		else
		{
	...
	public void SorryWeaponAndArmorExitClick()
	{
		...
		if (sorryArmorHatRemoved)
		{
			Storager.setInt("RemovedArmorHat_CoinsCountToCompensate", 0, false);
		}
		else if (sorryGearRemoved)
		{
			Storager.setInt(Defs.GearRemoved_GemsCountToCompensate, 0, false);
		}
		else
		{
