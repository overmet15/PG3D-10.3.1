...
public static class CryptoPlayerPrefs
{
	...
	private static string SaltString
	{
		get
		{
			if (string.IsNullOrEmpty(_saltString))
			{
				_saltString = salt.ToString();
			}
			return _saltString;
		}
	}
	...
	public static void SetInt(string key, int val)
	{
		...
		if (_useRijndael)
		{
			PlayerPrefs.SetString(text, encrypt(text, value.ToString()));
		}
		else
		{
	...
	private static string hashedKey(string key)
	{
		...
		string value;
		if (keyHashs.TryGetValue(key, out value))
		{
			return value;
		}
	...
	private static SymmetricAlgorithm getRijndaelForKey(string key)
	{
		...
		if (!rijndaelDict.TryGetValue(key, out value))
		{
			...
			value = Rijndael.Create();
			value.Key = rfc2898DeriveBytes.GetBytes(32);
			value.IV = rfc2898DeriveBytes.GetBytes(16);
			rijndaelDict.Add(key, value);
		}
		return value;
	}
