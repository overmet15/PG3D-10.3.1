...
public sealed class RewardWindowBase : MonoBehaviour
{
	...
	private IEnumerator SendAnalyticsOnShowCoroutine()
	{
		...
		Dictionary<string, string> parameters = new Dictionary<string, string> { { "Show Event", EventTitle } };
		...
		FlurryPluginWrapper.LogEventAndDublicateToConsole("Virality", parameters);
	}
	...
	private void SetButtonPositionsAndActive()
	{
		if (hideButton != null)
		{
			hideButton.gameObject.SetActive(!CollectOnlyNoShare && ShowLoginsButton && !Device.isPixelGunLow);
		}
		if (!ShowLoginsButton)
		{
			return;
	...
	public void HandleContinueButtonNoHide()
	{
		...
		if (TwitterController.TwitterSupported && TwitterController.IsLoggedIn && TwitterController.Instance.CanPostStatusUpdateWithPriority(twitterPriority))
		{
			...
			FlurryPluginWrapper.LogEventAndDublicateToConsole("Virality", new Dictionary<string, string>
			{
