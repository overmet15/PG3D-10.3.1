...
namespace Rilisoft
{
	public sealed class QuestProgress : IDisposable
	{
		...
		private static void ParseQuests(Dictionary<string, object> rawQuests, long? dayOption, Difficulty[] allowedDifficulties, IDictionary<int, List<QuestBase>> actualResult)
		{
			...
			{
				dictionary2 = FilterQuests(rawQuests, allowedDifficulties, actualResult);
			}
		...
		private IList<QuestBase> GetActiveQuestsBySlot(int slot)
		{
			List<QuestBase> activeQuestsBySlotReference = GetActiveQuestsBySlotReference(slot);
		...
		private static IDictionary<int, List<Dictionary<string, object>>> FilterQuests(Dictionary<string, object> rawQuests, Difficulty[] allowedDifficulties, IDictionary<int, List<QuestBase>> existingQuests)
		{
			...
			while (enumerator2.MoveNext())
			{
				...
				List<QuestBase> existingQuestList;
				Difficulty chosenDifficulty = ((!existingQuests.TryGetValue(key2, out existingQuestList) || existingQuestList.Count <= 0) ? list[num] : existingQuestList[0].Difficulty);
				...
				else
				{
					if (list3.Count > 1)
					{
						list3.RemoveAll((KeyValuePair<string, Dictionary<string, object>> kv) => kv.Key.Equals(existingQuestList.Map((List<QuestBase> l) => l.FirstOrDefault()).Map((QuestBase q) => q.Id), StringComparison.OrdinalIgnoreCase));
					}
					...
					if (existingQuestList.Map((List<QuestBase> l) => l.Count == 0, true))
					{
		...
		private static ConnectSceneNGUIController.RegimGame? ExtractModeFromQuestdescription(Dictionary<string, object> q, bool restore, string questId)
		{
			...
			ConnectSceneNGUIController.RegimGame[] array = GetSupportedModes();
			if ("killInMode".Equals(questId, StringComparison.OrdinalIgnoreCase))
			{
				array = array.Except(new ConnectSceneNGUIController.RegimGame[1] { ConnectSceneNGUIController.RegimGame.TimeBattle }).ToArray();
			}
			object value2;
			if (!q.TryGetValue("modes", out value2))
			{
				return (array.Length <= 0) ? null : new ConnectSceneNGUIController.RegimGame?(array[UnityEngine.Random.Range(0, array.Length - 1)]);
			}
			List<object> list = value2 as List<object>;
			if (list == null)
			{
			...
			return array2[UnityEngine.Random.Range(0, array2.Length - 1)];
		}
		...
		private void HandleSocialInteraction(object sender, SocialInteractionEventArgs e)
		{
			if (Defs.IsDeveloperBuild)
			{
				Debug.Log("HandleSocialInteraction()");
			}
