...
namespace Rilisoft
{
	public sealed class LevelCompleteScript : MonoBehaviour
	{
		...
		private void GiveAwardForCampaign()
		{
			...
			CampaignLevel levelBySceneName = LevelBox.GetLevelBySceneName(CurrentCampaignGame.levelSceneName);
			string text = ((levelBySceneName == null || levelBySceneName.localizeKeyForLevelMap == null) ? "FARM" : (LocalizationStore.GetByDefault(levelBySceneName.localizeKeyForLevelMap) ?? "FARM"));
			text = text.Replace("\n", " ");
			string twitterStatus = string.Format("All enemies {0} {1} are defeated in @PixelGun3D! Join my adventures now! #pixelgun3d #pixelgun #pg3d http://goo.gl/8fzL9u", levelBySceneName.predlog, text);
			...
			if (!Defs.IsSurvival)
			{
				...
				if (!_gameOver)
				{
					...
					try
					{
						...
						{
							List<string> second = Storager.getString(Defs.LevelsWhereGetCoinS, false).Split(new char[1] { '#' }, StringSplitOptions.RemoveEmptyEntries).ToList();
							List<string> first = LevelBox.campaignBoxes.Find((LevelBox lb) => lb.name == CurrentCampaignGame.boXName).levels.Select((CampaignLevel level) => level.sceneName).ToList();
							_shouldShowAllSecretsCollectedRewardWindow = !first.Except(second).Any();
						}
					}
			...
			while (true)
			{
				...
				try
				{
					...
					if (_shouldShowAllStarsCollectedRewardWindow && _numOfRewardWindowsShown < 2)
					{
						...
						GameObject window2 = UnityEngine.Object.Instantiate(Resources.Load<GameObject>("NguiWindows/AllStarsNGUI"));
						...
						rwb2.twitterStatus = () => string.Format("I've got all the stars in {0} in @PixelGun3D! Play now and try to get them! #pixelgun3d #pixelgun #pg3d http://goo.gl/8fzL9u", BoxNameForTwitter());
						rwb2.EventTitle = "All Stars";
						AllStarsRewardSettings asrs2 = window2.GetComponent<AllStarsRewardSettings>();
					...
					if (_shouldShowAllSecretsCollectedRewardWindow && _numOfRewardWindowsShown < 2)
					{
						...
						GameObject window = UnityEngine.Object.Instantiate(Resources.Load<GameObject>("NguiWindows/AllSecretsNGUI"));
						...
						rwb.HasReward = false;
						rwb.twitterStatus = () => string.Format("I've found all coins in {0} in @PixelGun3D! Play now and try to find them! #pixelgun3d #pixelgun #pg3d http://goo.gl/8fzL9u", BoxNameForTwitter());
						rwb.EventTitle = "All Coins";
						AllSecretsRewardSettings asrs = window.GetComponent<AllSecretsRewardSettings>();
